	...
	stages {
		...
		stage ("Deployment") {
			parallel {
				stage("Staging") {
					steps {
						sh "scp -i '/Users/toonday/Downloads/jenkins-demo.pem' **/target/*.war ssh ec2-user@${params.tomcat_staging}:/var/lib/tomcat7/webapp"
					}
					post {
						success {
				...
				stage("Production") {
					steps {
						...
						sh "scp -i '/Users/toonday/Downloads/jenkins-demo.pem' **/target/*.war ssh ec2-user@${params.tomcat_prod}:/var/lib/tomcat7/webapp"
					}
					post {
						success {
